...
{   // debug($_POST);
    ...
    ajouterProduitDansPanier($produit['nom'],$_POST['id_produit'],$_POST['quantite'],$produit['prix']);
}
//--- VIDER PANIER ---//
...
if(isset($_POST['payer']))
{
    for($i=0 ;$i < count($_SESSION['panier']['id_produit']) ; $i++)
    {
        ...
        $produit = $resultat->fetch_assoc();
        if($produit['quantite'] < $_SESSION['panier']['quantite'][$i])
        {
            $contenu .= '<hr><div class="erreur">Stock Restant: ' . $produit['quantite'] . '</div>';
            ...
            if($produit['quantite'] > 0)
            {
                ...
                $_SESSION['panier']['quantite'][$i] = $produit['quantite'];
            }
            else
            {
    ...
    if(!isset($erreur))
    {
        ...
        for($i = 0; $i < count($_SESSION['panier']['id_produit']); $i++)
        {
            executeRequete("INSERT INTO commande_produit(id_commande, id_produit, quantite, prix) VALUES ($id_commande, " . $_SESSION['panier']['id_produit'][$i] . "," . $_SESSION['panier']['quantite'][$i] . "," . $_SESSION['panier']['prix'][$i] . ")");
        }
        unset($_SESSION['panier']);
...
echo "<tr><th>Nomd</th><th>Produit</th><th>Quantité</th><th>Prix Unitaire</th></tr>";
...
else
{
    ...
    echo "<tr><td colspan='5'><a href='?action=vider' class=\"btn btn-primary\">Vider mon panier</a></td></tr>";
}
echo "</table><br>";
